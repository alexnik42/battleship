...
int check_surrounding(t_map *computer_map, int row, int col)
{
	...
	if (computer_map->map[row][col] == SHIP)
		return (0);

	if (row > 0 && col > 0)
	{
		if (computer_map->map[row - 1][col - 1] == SHIP)
			return (0);
	}
	if (row > 0)
	{
		if (computer_map->map[row - 1][col] == SHIP)
			return (0);
	}
	...
	if (col < MAP_SIZE - 1)
	{
		if (computer_map->map[row][col + 1] == SHIP)
			return (0);
	}
